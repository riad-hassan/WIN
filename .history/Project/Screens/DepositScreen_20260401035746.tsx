...
const handleSelectGateway = async (item) => {
  ...
  if (item.name === "bKash") navigation.navigate("BkashScreen", { depositId });
  else if (item.name === "Nagad") navigation.navigate("NagadScreen", { depositId });
  else if (item.name === "Rocket") navigation.navigate("RocketScreen", { depositId });
  else if (item.name === "Upay") navigation.navigate("UpayScreen", { depositId });
};
...
useEffect(() => {
  const checkLogin = async () => {
    ...
    if (!session) {
      Alert.alert("Please login first");
      navigation.navigate("Login"); // তোমার login screen নাম দিন
    }
  };
  checkLogin();
}, []);
