...
// ==============================
// LEFT + BOTTOM SPEED STREAKS
// ==============================
const streaks = [];
const streakGraphics = new PIXI.Graphics();
camera.addChild(streakGraphics);
...
const s = Math.min(app.screen.width / 600, 1.5);
plane.scale.set(s);
...
// ==============================
// RIGHT SIDE LIVE BET PANEL
// ==============================
const betPanel = new PIXI.Container();
app.stage.addChild(betPanel);

const panelBg = new PIXI.Graphics();
panelBg.beginFill(0x111827, 0.70);
panelBg.drawRoundedRect(0, 0, 140, app.screen.height - 20, 12);
panelBg.endFill();
betPanel.addChild(panelBg);

betPanel.x = app.screen.width - 150;
betPanel.y = 10;

const panelTitle = new PIXI.Text("LIVE BETS", {
  fill: "#ffffff",
  fontSize: 14,
  fontWeight: "bold"
});
panelTitle.x = 15;
panelTitle.y = 10;
betPanel.addChild(panelTitle);

const betListContainer = new PIXI.Container();
betListContainer.y = 35;
betPanel.addChild(betListContainer);

let fakeBets = [];

// generate random name
function randomName() {
  const names = ["Rafi", "Sakib", "Naim", "Fahim", "Jubayer", "Rony", "Arif", "Tanvir", "Hasan", "Mithun"];
...
function addFakeBet() {
  ...
  const multi = multiplier.toFixed(2);

  const row = new PIXI.Container();

  const rowBg = new PIXI.Graphics();
  rowBg.beginFill(0x0b1220, 0.55);
  rowBg.drawRoundedRect(0, 0, 125, 28, 8);
  rowBg.endFill();
  row.addChild(rowBg);
  ...
  amountText.anchor.set(1, 0);
  amountText.x = 120;
  amountText.y = 6;
  row.addChild(amountText);

  row.y = fakeBets.length * 32;
  betListContainer.addChild(row);

  fakeBets.push(row);

  // limit 10 rows
  if (fakeBets.length > 10) {
    betListContainer.removeChild(fakeBets[0]);
    fakeBets.shift();

    // re-position
    fakeBets.forEach((r, i) => {
      r.y = i * 32;
    });
  }
}

// every 1 sec add new fake bet
setInterval(() => {
  if (!crashed && phase !== "takeoff") {
    addFakeBet();
  }
}, 900);
...
streaks.forEach((s, i) => {
  // move down-right style
  s.x -= s.speed * (1 + multiplier * 0.15);
  s.y += s.speed * 1.5 * (1 + multiplier * 0.10);

  // draw streak line
  streakGraphics.lineStyle(2, 0xffffff, s.alpha);
  streakGraphics.moveTo(s.x, s.y);
  streakGraphics.lineTo(s.x + s.length, s.y - s.length * 0.2);

  // reset if out
  if (s.x < -50 || s.y > app.screen.height + 50) {
    streaks[i] = spawnStreak();
    streaks[i].x = app.screen.width + Math.random() * 100;
    streaks[i].y = Math.random() * (app.screen.height * 0.6);
  }
});
