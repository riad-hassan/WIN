...
// keep center
let camDriftX = Math.sin(Date.now() * 0.0015) * 6
let camDriftY = Math.cos(Date.now() * 0.0012) * 4

let camShake = Math.sin(Date.now() * 0.01) * (multiplier * 0.3)

camera.x = -(app.screen.width * (zoom - 1)) / 2 + camDriftX + camShake
camera.y = -(app.screen.height * (zoom - 1)) / 2 + camDriftY + camShake
...
// 🎯 natural float (IMPORTANT)
let floatWave = Math.sin(Date.now() * 0.004) * (1.5 + multiplier * 0.2)

// 🎯 micro shake
let microShake = (Math.random() - 0.5) * 0.8

plane.y = y + floatWave + microShake
  ...
  let wobble = Math.sin(Date.now()*0.005) * 0.03
plane.rotation += (targetRotation + wobble - plane.rotation) * 0.08
}
...
// 🎯 parallax boost
gridOffsetX -= (1 + multiplier * 0.8)
gridOffsetY += (5 + Math.pow(multiplier,1.6)*2)
...
let dynamicGlow = 8 + Math.sin(Date.now()*0.01)*2

glow.lineStyle(dynamicGlow,0xff3b30,0.25)
...
// ---------- MULTIPLIER ----------
let growth = 1.0015 + Math.random()*0.0015
multiplier *= (1 + growth)
