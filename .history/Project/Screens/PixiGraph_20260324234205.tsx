...
// ---------- CRUISE ----------
  ...
  const speed = 1.2 + multiplier * 0.3
  ...
  // 🔥 real curve
  const baseY = app.screen.height - 30
  const curveStrength = 65

  let curveY = baseY - Math.pow(multiplier, 2.1) * curveStrength

  // 🔥 clamp inside screen
  y = Math.max(80, curveY)

  // ✅ plane ALWAYS inside graph
  plane.x = PLANE_X
  plane.y = y

  // ✅ add point EXACTLY where plane is
  points.push({
    x: PLANE_X,
    y: y
  })
