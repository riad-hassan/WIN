...
const playerUI = new PIXI.Container()
app.stage.addChild(playerUI)

// position (right side)
playerUI.x = app.screen.width - 40
playerUI.y = app.screen.height / 2
...
function randomPlayer(){
  return Math.floor(200 + Math.random()*800) // 200 - 1000
}
...
playerUI.addChild(playerGraphics)
...
function drawPlayers(){
  ...
  players.forEach((p, i)=>{
    ...
    // circle
    playerGraphics.beginFill(0xffffff, 0.15)
    playerGraphics.drawCircle(0, p.y, 14)
    playerGraphics.endFill()

    // inner active dot
    playerGraphics.beginFill(0x4cff88, 1)
    playerGraphics.drawCircle(0, p.y, 6)
    playerGraphics.endFill()

    // text
    if(!playerTexts[i]){
      const t = new PIXI.Text(p.value.toString(), {
        fill:"#ffffff",
        fontSize:12
      })
      t.anchor.set(1, 0.5)
...
// remove top + add new bottom
if(players[0].y < -30){
  players.shift()

  players.push({
    value: randomPlayer(),
    y: players[players.length-1].y + 35
  })
...
// ---------- CRUISE ----------
  ...
  let t = multiplier - 1   // 🔥 start from 0 (important)

  // smooth increasing curve (same style feel)
  let curve = Math.pow(t, 1.4)

  y = lastY - curve * 10   // 🔥 komale barale plan upore nice hby

  // limit (screen এর বাইরে না যায়)
  y = Math.max(40, y)

  // 👉 important (REAL curve grow)
  points.push({ x: plane.x, y: y })

  plane.y = y

  // rotation
  let targetRotation = -0.2 + Math.min(0.35, Math.pow(t, 0.5) * 0.2)
