...
const handleRegister = async () => {
  ...
  try {
    ...
    const { data: authData, error: authError } = await supabase.auth.signUp({
      email: email || `${phone}@temp.com`, // phone হলে fake email
      password: pass,
    });
    ...
    if (referral) {
      ...
      if (refUser) {
        referredBy = refUser.auth_id;
      }
    }

    // 🧾 3. users table এ insert
    ...
    // 👤 4. profiles update (role + referral)
