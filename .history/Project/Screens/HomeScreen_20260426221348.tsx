...
const [match, setMatch] = useState(matchPool[0]);
  const [runs, setRuns] = useState(107);
  const [wickets, setWickets] = useState(7);
  const [overs, setOvers] = useState(17.4);
  ...
  const overRef = useRef(overs);

  // 🔥 overs calculation
  const updateOvers = () => {
    let current = overRef.current;
    let overInt = Math.floor(current);
    let balls = Math.round((current - overInt) * 10);
    ...
    let newOvers = parseFloat(`${overInt}.${balls}`);
    overRef.current = newOvers;
    setOvers(newOvers);
  };

  // 🔥 fake score update system
  useEffect(() => {
    const interval = setInterval(() => {
      ...
      // Run increase chance
      if (random < 0.65) {
        setRuns((prev) => prev + Math.floor(Math.random() * 7)); // 0-6
      }

      // wicket chance
      if (random > 0.92 && wickets < 10) {
      ...
      // odds slight changes
      setOdds((prev) => ({
        w1: +(prev.w1 + (Math.random() * 0.05 - 0.02)).toFixed(3),
        draw: +(prev.draw + (Math.random() * 1 - 0.5)).toFixed(0),
        w2: +(prev.w2 + (Math.random() * 0.2 - 0.1)).toFixed(2),
      }));
    }, 2500);
  ...
  // 🔥 match change system (every 20 sec)
  useEffect(() => {
    const matchInterval = setInterval(() => {
      ...
      // reset score
      setRuns(Math.floor(Math.random() * 80) + 20);
      setWickets(Math.floor(Math.random() * 5));
      overRef.current = parseFloat((Math.random() * 10 + 5).toFixed(1));
      setOvers(overRef.current);

      setOdds({
        w1: +(Math.random() * 1 + 1.1).toFixed(3),
        draw: 25,
        w2: +(Math.random() * 5 + 2).toFixed(2),
      });
    }, 20000);
