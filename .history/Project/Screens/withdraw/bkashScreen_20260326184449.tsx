...
export default function WbkashScreen() {
 const { balance, setBalance, addWithdraw } = useContext(DepositContext);
  ...
  const handleWithdraw = () => {
    ...
    if (!number) {
      Alert.alert("Error", "bKash নাম্বার দিন");
      return;
    }

    if (number.length < 11) {
      Alert.alert("Error", "Valid bKash নাম্বার দিন");
      return;
    }
...
addWithdraw({
  ...
  }) // তোমার main screen name এখানে বসাও
      }
    ]);
  };
  ...
  <Text> </Text>
      ...
      {/* CONTINUE BUTTON */}
      <TouchableOpacity style={styles.continueBtn} onPress={handleWithdraw}>
        <Text style={styles.continueText}>
          {amount !== "" ? `WITHDRAW ৳${numAmount}` : "Enter amount to continue"}
        </Text>
      </TouchableOpacity>
