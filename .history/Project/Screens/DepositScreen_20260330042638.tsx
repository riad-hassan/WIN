...
const { fetchBalance } = useContext(DepositContext);
...
const handleDeposit = async (amount) => {
  ...
  // 1️⃣ Insert deposit row
  const { data, error } = await supabase
    .from("deposits")
    .insert([{
      user_id: Number(userId),
      method: selectedGateway.name,
      gateway_number: selectedGateway.number,
      amount,
      status: "Pending"
    }])
    .select();
  ...
  // 2️⃣ Update user balance (simulate approval)
  const { data: userData, error: balanceError } = await supabase
  .from("users")
  .update({ balance: supabase.literal(`balance + ${amount}`) })
  .eq("id", Number(userId));

  if (balanceError) return Alert.alert("Error", balanceError.message);

  // 3️⃣ Refresh context balance
  fetchBalance();

  // 4️⃣ Navigate to method screen
  if (selectedGateway.name === "bKash") navigation.navigate("bkash", { depositId });
  else if (selectedGateway.name === "Nagad") navigation.navigate("Nagad", { depositId });
  else if (selectedGateway.name === "Rocket") navigation.navigate("Rocket", { depositId });
  else if (selectedGateway.name === "Upay") navigation.navigate("Upay", { depositId });
};
            ...
            {gateways.map((item) => (
              <TouchableOpacity
                ...
                style={[
                  ...
                  onPress={() => handleDeposit(amount)} // Or any amount
              >
