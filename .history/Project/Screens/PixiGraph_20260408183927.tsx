...
export default function PixiGraph({ onMessage, width=380, height=240, webRef }) {
 const [planeBase64, setPlaneBase64] = useState("");

useEffect(() => {
  (async () => {
    try {
      const base64 = await RNFS.readFileAssets("plane.png", "base64");
      setPlaneBase64(`data:image/png;base64,${base64}`);
    } catch (e) {
      console.log("Plane load error:", e);
    }
  })();
}, []);
if (!planeBase64) {
...
// ---------- PLANE ----------
window.ReactNativeWebView.postMessage("PIXIGRAPH START");
const planeTexture = await PIXI.Assets.load("${planeBase64}");
const plane = new PIXI.Sprite(planeTexture);

plane.anchor.set(0.5, 0.5)

// 🔥 scale use করো
plane.scale.set(0.3) // try 0.2 / 0.4 adjust
