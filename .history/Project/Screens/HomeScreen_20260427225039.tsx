...
const bottomMenuItems = [
 ...
 const renderIcon = (item) => (
  ...
  const games = [ 
    ...
    { id: "7", title: "CRASH", image: require("../Screens/assets/crash.png"), action: "screen", screen: "AVIATOR GAME" },
...
const matchPool = [
  { league: "Indian Premier League", team1: "Chennai Super Kings", team2: "Mumbai Indians" },
  { league: "Indian Premier League", team1: "Royal Challengers Bangalore", team2: "Kolkata Knight Riders" },
  { league: "Indian Premier League", team1: "Rajasthan Royals", team2: "Delhi Capitals" },
  { league: "Indian Premier League", team1: "Sunrisers Hyderabad", team2: "Punjab Kings" },

  { league: "Bangladesh Premier League", team1: "Comilla Victorians", team2: "Sylhet Strikers" },
  { league: "Bangladesh Premier League", team1: "Fortune Barishal", team2: "Rangpur Riders" },
  { league: "Bangladesh Premier League", team1: "Dhaka Dominators", team2: "Chattogram Challengers" },
  { league: "Bangladesh Premier League", team1: "Khulna Tigers", team2: "Durbar Rajshahi" },

  { league: "Pakistan Super League", team1: "Lahore Qalandars", team2: "Karachi Kings" },
  { league: "Pakistan Super League", team1: "Islamabad United", team2: "Peshawar Zalmi" },
  { league: "Pakistan Super League", team1: "Multan Sultans", team2: "Quetta Gladiators" },

  { league: "Big Bash League", team1: "Sydney Sixers", team2: "Melbourne Stars" },
  { league: "Big Bash League", team1: "Sydney Thunder", team2: "Adelaide Strikers" },
  { league: "Big Bash League", team1: "Brisbane Heat", team2: "Perth Scorchers" },
  { league: "Big Bash League", team1: "Hobart Hurricanes", team2: "Melbourne Renegades" },

  { league: "Caribbean Premier League", team1: "Trinbago Knight Riders", team2: "Guyana Amazon Warriors" },
  { league: "Caribbean Premier League", team1: "Jamaica Tallawahs", team2: "Barbados Royals" },
  { league: "Caribbean Premier League", team1: "St Lucia Kings", team2: "Antigua and Barbuda Falcons" },

  { league: "T20 World Cup", team1: "India", team2: "Pakistan" },
  { league: "T20 World Cup", team1: "Bangladesh", team2: "Sri Lanka" },
  { league: "T20 World Cup", team1: "Australia", team2: "England" },
  { league: "T20 World Cup", team1: "South Africa", team2: "New Zealand" },
  { league: "T20 World Cup", team1: "West Indies", team2: "Afghanistan" },

  { league: "Asia Cup", team1: "India", team2: "Sri Lanka" },
  { league: "Asia Cup", team1: "Bangladesh", team2: "Pakistan" },
  { league: "Asia Cup", team1: "Afghanistan", team2: "Nepal" },

  { league: "ICC Champions Trophy", team1: "England", team2: "South Africa" },
  { league: "ICC Champions Trophy", team1: "Australia", team2: "New Zealand" },

  { league: "International Friendly", team1: "Bangladesh", team2: "Zimbabwe" },
  { league: "International Friendly", team1: "Ireland", team2: "Scotland" },
];

function generateRandomMatch() {
  return matchPool[Math.floor(Math.random() * matchPool.length)];
}

function generateOdds() {
  return {
    w1: +(Math.random() * 1.3 + 1.05).toFixed(3),
    draw: 25,
    w2: +(Math.random() * 5 + 2).toFixed(2),
  };
}
...
function generateScore() {
  return {
    ...
    overs: parseFloat((Math.random() * 10 + 5).toFixed(1)),
    target: Math.floor(Math.random() * 80) + 140,
    inning: 1,
    lastBalls: ["-", "-", "-", "-", "-", "-"],
  };
}

function updateOversValue(currentOvers) {
  let overInt = Math.floor(currentOvers);
  let balls = Math.round((currentOvers - overInt) * 10);
  ...
  return parseFloat(`${overInt}.${balls}`);
}

function calculateRunRate(runs, overs) {
  let overInt = Math.floor(overs);
  let balls = Math.round((overs - overInt) * 10);

  let totalBalls = overInt * 6 + balls;
  if (totalBalls === 0) return "0.00";

  let rr = (runs / totalBalls) * 6;
  return rr.toFixed(2);
}
...
function generateBallEvent() {
  ...
  if (rand < 0.12) return "W";
  if (rand < 0.22) return "0";
  if (rand < 0.38) return "1";
  if (rand < 0.52) return "2";
  if (rand < 0.62) return "3";
  if (rand < 0.78) return "4";
  if (rand < 0.92) return "6";
  return "1";
}

const LiveMatchCard = ({ data, blinkAnim }) => {
  const rr = calculateRunRate(data.runs, data.overs);

  return (
    <View style={styles.liveCard}>
      <View style={styles.headerRow}>
        <Text style={styles.eventText}>🏏 Event in progress</Text>
      ...
      <Text style={styles.league}>{data.match.league}</Text>

      <View style={styles.teamRow}>
        <Text style={styles.team}>{data.match.team1}</Text>
        <Text style={styles.score}>0/0</Text>
      </View>

      <View style={styles.teamRow}>
        <Text style={styles.team}>{data.match.team2}</Text>
      ...
      <View style={styles.infoRow}>
        <Text style={styles.infoText}>Innings: {data.inning}</Text>
        <Text style={styles.infoText}>RR: {rr}</Text>
        <Text style={styles.infoText}>Target: {data.target}</Text>
      </View>

      {/* Last 6 Balls */}
      <View style={styles.lastBallRow}>
        <Text style={styles.lastBallTitle}>Last 6:</Text>
        <View style={styles.ballBoxRow}>
          {data.lastBalls.map((ball, i) => (
            <View
              key={i}
              style={[
                styles.ballBox,
                ball === "4" && styles.ballFour,
                ball === "6" && styles.ballSix,
                ball === "W" && styles.ballWicket,
              ]}
            >
      ...
      <View style={styles.oddsRow}>
        <TouchableOpacity style={styles.oddsBtn}>
          <Text style={styles.oddsText}>W1{"\n"}{data.odds.w1}</Text>
        </TouchableOpacity>

        <TouchableOpacity style={styles.oddsBtn}>
          <Text style={styles.oddsText}>DRAW{"\n"}{data.odds.draw}</Text>
        </TouchableOpacity>

        <TouchableOpacity style={styles.oddsBtn}>
          <Text style={styles.oddsText}>W2{"\n"}{data.odds.w2}</Text>
        </TouchableOpacity>
      </View>
    </View>
  );
...
const [liveMatches, setLiveMatches] = useState([
    { id: 1, match: generateRandomMatch(), ...generateScore(), odds: generateOdds() },
    { id: 2, match: generateRandomMatch(), ...generateScore(), odds: generateOdds() },
    { id: 3, match: generateRandomMatch(), ...generateScore(), odds: generateOdds() },
    { id: 4, match: generateRandomMatch(), ...generateScore(), odds: generateOdds() },
  ]);

  const oversRef = useRef(liveMatches.map((m) => m.overs));
  ...
  // 🔥 Runs/Wickets/Overs update
  useEffect(() => {
    const interval = setInterval(() => {
      setLiveMatches((prev) =>
        prev.map((item, index) => {
          let newRuns = item.runs;
          let newWickets = item.wickets;

          // overs update
          let newOvers = updateOversValue(oversRef.current[index]);
          oversRef.current[index] = newOvers;

          // ball event
          const ballEvent = generateBallEvent();

          let updatedBalls = [...item.lastBalls];
          updatedBalls.shift();
          updatedBalls.push(ballEvent);

          // update runs/wicket
          if (ballEvent === "W") {
            if (newWickets < 10) newWickets += 1;
          } else {
            newRuns += parseInt(ballEvent);
          }

          // 20 overs complete -> innings change/reset
          if (Math.floor(newOvers) >= 20 || newWickets >= 10) {
            // inning 1 finished -> inning 2
            if (item.inning === 1) {
              return {
                ...item,
                inning: 2,
              ...
              // inning 2 finished -> new match start
              const newMatch = generateRandomMatch();
              const newScore = generateScore();

              oversRef.current[index] = newScore.overs;
          ...
          // odds update small movement
          const newOdds = {
            w1: +(item.odds.w1 + (Math.random() * 0.04 - 0.02)).toFixed(3),
            draw: item.odds.draw,
            w2: +(item.odds.w2 + (Math.random() * 0.2 - 0.1)).toFixed(2),
          };
  ...
  // ⚠️ Demo match change time (Default 1 hour)
  useEffect(() => {
    const matchChangeInterval = setInterval(() => {
      setLiveMatches((prev) =>
        prev.map((item, index) => {
          const newMatch = generateRandomMatch();
          const newScore = generateScore();
    ...
    return () => clearInterval(matchChangeInterval);
  }, []);
...
useEffect(() => {
  ...
  const fetchLinks = async () => {
    const { data, error } = await supabase
      .from("Help Contact")
      .select("*")
      .in("key", ["telegram_link", "whatsapp_link"]);
    ...
    data.forEach((item) => {
      ...
      if (item.key === "whatsapp_link") setWhatsappLink(item.value);
    });
  };
  ...
  // ✅ REALTIME SUBSCRIBE
  channel = supabase
    .channel("help-contact-live")
    .on(
      "postgres_changes",
      { event: "*", schema: "public", table: "Help Contact" },
      (payload) => {
        console.log("Realtime update:", payload);
        fetchLinks(); // reload data instantly
      }
    )
    .subscribe();
...
<View style={styles.gamesContainer}>  
  {games.map((game) => (
    ...
    >
      ...
      {(game.title === "AVIATOR GAME" || game.title === "CRASH" || game.title === "CRAZY TIME") && (
        <Animated.View style={[styles.liveBadge, { opacity: blinkAnim }]}>
          <Text style={styles.liveText}>🔴 LIVE</Text>
        </Animated.View>
      )}
    ...
    >
      <ScrollView style={{ flex: 1 }} showsVerticalScrollIndicator={false}>
        <View style={styles.sectionHeader}>
          ...
          <TouchableOpacity>
            <Text style={styles.moreText}>MORE LIVE »</Text>
          </TouchableOpacity>
        </View>

        {/* 4 Cards */}
        {liveMatches.map((item) => (
          <LiveMatchCard key={item.id} data={item} blinkAnim={blinkAnim} />
        ))}
