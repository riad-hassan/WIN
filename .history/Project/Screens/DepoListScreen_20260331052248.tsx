...
export default function DepoListScreen() {
  ...
  useEffect(() => {
    ...
    const initialize = async () => {
      // 🔹 আগের সব লোকাল ডাটা মুছে দাও
      await AsyncStorage.clear();

      // 🔹 deposits state খালি করা
      setDeposits([]);
      setLoading(true);
      ...
      const fetchDeposits = async () => {
        try {
          ...
          if (error) {
            ...
            setDeposits([]); // error হলে deposits খালি রাখো
          } else {
            setDeposits(data || []);
          }
        } catch (err) {
      ...
      // প্রতি 20 সেকেন্ডে refresh
      interval = setInterval(fetchDeposits, 20000);
    };
