    ...
    .insert([
      {
        ...
        amount: 0,       // প্রথমে amount 0, পরে Method Screen এ আপডেট হবে
      }
    ]);

  if (error) console.log("DepositScreen insert error:", error.message);

  // Navigate
  if (item.name === "bKash") navigation.navigate("Bkash", { depositId: data[0].id });
  else if (item.name === "Nagad") navigation.navigate("Nagad", { depositId: data[0].id });
  else if (item.name === "Rocket") navigation.navigate("Rocket", { depositId: data[0].id });
  else if (item.name === "Upay") navigation.navigate("Upay", { depositId: data[0].id });
};
