...
if(phase === "takeoff"){
  takeoffProgress = planeX / (app.screen.width * 0.75)
}else{
  takeoffProgress = 1
}

// smooth curve
let zoomBase = 1 + takeoffProgress * 0.3

// multiplier boost
let zoomBoost = Math.pow(multiplier, 1.2) * 0.015

// final zoom
let targetZoom = zoomBase + zoomBoost

// subtle breathing
targetZoom += Math.sin(Date.now() * 0.003) * 0.02

// clamp
targetZoom = Math.min(targetZoom, 1.8)
...
if(phase === "takeoff"){
 ...
 points.push({
  ...
  plane.x = planeX + (app.screen.width / 2 - camera.pivot.x)
plane.y = y + (app.screen.height / 2 - camera.pivot.y)
