...
// ---------- ONLINE PLAYERS UI ----------
const onlinePlayersContainer = new PIXI.Container();
onlinePlayersContainer.x = app.screen.width - 10; // right margin
onlinePlayersContainer.y = 10; // top margin
onlinePlayersContainer.pivot.set(0,0);
camera.addChild(onlinePlayersContainer);
...
function updateOnlinePlayers(count) {
    // remove old children
    onlinePlayersContainer.removeChildren();

    // title text
    const title = new PIXI.Text("Online Players", {
        fill: "#ffffff",
        fontSize: 14,
        fontWeight: "bold"
    });
    title.anchor.set(0, 0);
    onlinePlayersContainer.addChild(title);

    // display circles for players (max 10 for demo)
    for(let i = 0; i < Math.min(count, 10); i++){
        const circle = new PIXI.Graphics();
        circle.beginFill(0x4ade80); // green color
        circle.drawCircle(0, 0, 6);
        circle.endFill();
        circle.x = (i % 5) * 15; // 5 per row
        circle.y = 20 + Math.floor(i / 5) * 15;
        onlinePlayersContainer.addChild(circle);
    }

    // count text
    const countText = new PIXI.Text(count + " players", {
        fill: "#ffffff",
        fontSize: 12
    });
    ...
    countText.x = 0;
    countText.y = 50;
    onlinePlayersContainer.addChild(countText);
}
// 🔥 Example: randomly update online players
setInterval(() => {
  const randomCount = Math.floor(Math.random() * 20) + 1;
  updateOnlinePlayers(randomCount);
}, 3000);
