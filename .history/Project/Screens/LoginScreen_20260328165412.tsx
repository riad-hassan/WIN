...
const handleLogin = async () => {
  ...
  try {
    let query = supabase.from("users").select("*").eq("password", pass);

    if (tab === "phone") query = query.eq("phone", phone);
    if (tab === "email") query = query.eq("email", email);
    if (tab === "user") query = query.eq("username", user);
