...
// subtle breathing (very important for realism)
targetZoom += Math.sin(Date.now() * 0.003) * 0.015
...
// ---------- REALISTIC SHAKE (DIRECTIONAL RECOIL) ----------

// intensity grows with multiplier
let intensity = Math.min(10, multiplier * 0.6)

// smooth decay
let decay = Math.max(0, 1 - multiplier * 0.08)

// top-left recoil direction + small randomness
let shakeX = (-intensity * 0.7 * decay) + (Math.random() - 0.5) * 0.8
let shakeY = (-intensity * 0.5 * decay) + (Math.random() - 0.5) * 0.6
...
camera.position.set(
  app.screen.width / 2 + shakeX,
  app.screen.height / 2 + shakeY
)
