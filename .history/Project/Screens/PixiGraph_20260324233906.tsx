...
// ---------- CAMERA ZOOM ----------
let zoom = 1 + Math.log(multiplier) * 0.02  // 🔥 reduced zoom
...
if(phase === "takeoff"){
  ...
  // 🐢 speed কমানো (slow takeoff)
let speed = 0.4 + (1 - progress) * 0.6
planeX += speed

// ✅ REAL AVIATOR CURVE (correct direction)
let curve = 1 - Math.cos(progress * Math.PI / 2)

// extra smooth (natural feel)
curve = Math.pow(curve, 1.3)
...
// ---------- CRUISE ----------
  ...
  // 🔥 graph scroll (left move)
  points.forEach(p => {
    p.x -= 1.5 + multiplier * 0.4
  })

  // 🔥 curve calculate (REAL SHAPE)
  const baseY = app.screen.height - 20
  const curveStrength = 70   // 🔥 কমালে smooth হবে

  let curveY = baseY - Math.pow(multiplier, 2.2) * curveStrength
  y = Math.max(70, curveY)

  // 🔥 plane position (same as graph point)
  const planeXFixed = app.screen.width * 0.7
  const planeYFixed = y   // 🔥 KEY FIX

  plane.x = planeXFixed
  plane.y = planeYFixed

  // 🔥 নতুন point add exactly plane position এ
  points.push({
    x: planeXFixed,
    y: planeYFixed
  })
  ...
  // 🔥 rotation natural
  plane.rotation = -0.2 + multiplier * 0.015
}
...
// limit points
if(points.length > 80){
  points.splice(1,1) // 🔥 index 0 থাকবে (origin fixed)
}
...
let acceleration = 0.00015 * multiplier
