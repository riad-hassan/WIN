...
const [liveMatches, setLiveMatches] = useState([
  ...
  // 🔥 Runs/Wickets/Overs update every 3 sec
  useEffect(() => {
    const interval = setInterval(() => {
      setLiveMatches((prev) =>
        prev.map((item, index) => {
          ...
          // Runs update chance
          if (random < 0.7) {
            newRuns += Math.floor(Math.random() * 7); // 0-6
          }

          // Wicket chance
          if (random > 0.93 && newWickets < 10) {
            newWickets += 1;
          }
  ...
  // ⚠️ Match change interval (Demo) - default 30 minutes
  useEffect(() => {
    const matchChangeInterval = setInterval(() => {
      ...
      oversRef.current = oversRef.current.map(() =>
        parseFloat((Math.random() * 10 + 5).toFixed(1))
      );
    }, 2 * 60 * 60 * 1000); // 30 minutes
