...
// ---------- CAMERA ZOOM & SHAKE ----------
let takeoffProgress = phase === "takeoff" ? planeX / (app.screen.width * 0.75) : 1

// base zoom increases as plane takes off
let zoomBase = 1 + takeoffProgress * 0.2  // subtle zoom

// small shake effect
let shakeX = Math.sin(Date.now() * 0.01) * 2
let shakeY = Math.cos(Date.now() * 0.01) * 2

// multiplier boost (optional for dynamic zoom during cruise)
let zoomBoost = phase === "cruise" ? Math.pow(multiplier, 1.2) * 0.01 : 0

// final zoom
let targetZoom = zoomBase + zoomBoost

// clamp zoom
targetZoom = Math.min(targetZoom, 1.2)  // max subtle zoom
...
// apply shake
camera.position.set(app.screen.width / 2 + shakeX, app.screen.height / 2 + shakeY)
