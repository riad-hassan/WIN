...
document.addEventListener("message", (event) => {
  ...
  if (data.type === "SET_CRASH") {
    crashPoint = data.value; // backend থেকে নতুন crash point সেট হবে
  }
});
...
// ---------- PLANE ----------
const planeTexture = PIXI.Texture.from("https://i.ibb.co/sd6tdTwH/plane.png")
...
if(r < 0.89){
  // 🔥 89% (1 – 3)
  crashPoint = 1 + Math.random() * 2
}
else if(r < 0.94){
  // 🔥 5% (1 – 5)
  crashPoint = 1 + Math.random() * 4
}
else if(r < 0.99){
  // 🔥 5% (1 – 7)
  crashPoint = 1 + Math.random() * 6
}
else{
  // 🔥 1% (1 – 15)
  crashPoint = 1 + Math.random() * 14
}
...
document.addEventListener("message", (event) => {
  ...
  fetchCrash(); // backend থেকে crash point আনা হচ্ছে
}

if(data.type === "CRASH"){
  fetch("http://YOUR_IP:3000/consume", { method: "POST" });
}
...
zoom = Math.min(zoom, 1.6)   // 🔥 clamp hard
