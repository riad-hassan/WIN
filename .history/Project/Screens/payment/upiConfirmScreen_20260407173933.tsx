...
const [upayNumber, setUpayNumber] = useState("");
    ...
    useEffect(() => {
      const fetchRandomNumber = async () => {
        const { data, error } = await supabase
          ...
          .select("number")
          .eq("method", "upay");  // method change করলে Nagad/Rocket/Upay ও fetch হবে
        ...
        if (error) {
          ...
          setUpayNumber(data[randomIndex].number);
        }
      };
  ...
  return (
    ...
    <View style={styles.container}>
      ...
      <View style={styles.topBox}>
        <Text style={styles.amountText}>₹{amount} INR</Text>
        <Text style={styles.timer}>{formatTime()}</Text>
      </View>
      

      {/* STEP 1 */}
      <Text style={styles.step}>👉 Send to this UPI Account</Text>
      ...
      <Text style={{color:"#fff", marginBottom:-5, marginTop: 5, fontSize:12}} > Your UPI ID (optional)</Text>
      <TextInput
