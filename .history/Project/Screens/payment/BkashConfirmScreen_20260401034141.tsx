...
const handleContinue = async () => {
    if (!amount || Number(amount) < 500) return Alert.alert("Minimum 500 BDT required");
    ...
    const { error } = await supabase
      ...
      .eq("id", depositId)
      .eq("user_id", user.id); // 🔹 শুধুমাত্র নিজের deposit update

    if (error) return Alert.alert("Error", error.message);

    navigation.navigate("BkashConfirmScreen", { depositId, amount });
  };
