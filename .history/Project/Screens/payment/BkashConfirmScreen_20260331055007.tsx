...
export default function BkashConfirmScreen() {
...
useEffect(() => {
  const checkPendingDeposit = async () => {
    ...
    if (data && data.length > 0) {
      setPendingDeposit(data[0]); // Pending Deposit আছে → দেখাবে
    } else {
      setPendingDeposit(null); // কোন Pending Deposit নেই → কিছু দেখাবে না
    }
  };
...
useEffect(() => {
  const fetchRandomNumber = async () => {
    const { data, error } = await supabase
      ...
      .select("number")
      .eq("method", "bkash");  // method change করলে Nagad/Rocket/Upay ও fetch হবে
    ...
    if (error) {
      ...
      setBkashNumber(data[randomIndex].number);
    }
  };
...
const handleSubmit = async () => {
  ...
  const { error } = await supabase
    ...
    .update({
      ...
      // ❌ status change করো না (already Pending)
    })
    .eq("id", depositId);
...
if (pendingDeposit) {
  return (
    <View style={{flex:1, justifyContent:'center', alignItems:'center', padding:20}}>
      <Text style={{fontSize:18, fontWeight:'bold', textAlign:'center'}}>
        Your Payment request submitted successfully!{"\n"}
        Please wait until it is approved or rejected.
      </Text>
      <ActivityIndicator size="large" color="#066b24ff" style={{marginTop:20}} />
    </View>
  );
}
  ...
  return (
    ...
    <View style={styles.container}>
      ...
      {/* STEP 1 */}
      <Text style={styles.step}>👉 Send to this bKash Number</Text>
      ...
      <Text style={{color:"#fff", marginBottom:-5, marginTop: 5, fontSize:12}} > Your bKash Number (optional)</Text>
      <TextInput
