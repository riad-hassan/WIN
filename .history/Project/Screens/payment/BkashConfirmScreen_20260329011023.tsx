...
export default function BkashConfirmScreen() {
  ...
  const bkashNumber = "01407956446"; // 👉 তুমি change করবা
  ...
  if (!permissionResult.granted) {
    Alert.alert("Permission required", "Gallery permission দরকার");
    return;
  }
...
const openCamera = async () => {
  ...
  if (!permissionResult.granted) {
    Alert.alert("Permission required", "Camera permission দরকার");
    return;
  }
...
const { addDeposit, setBalance } = useContext(DepositContext);
...
const handleSubmit = () => {
  ...
  if (!image) {
  Alert.alert("Error", "Screenshot upload করুন");
  return;
}
  ...
  return (
    <View style={styles.container}>
      ...
      <Text style={styles.step}>👉 Upload Payment Screenshot</Text>

<TouchableOpacity style={styles.uploadBox} onPress={pickImage}>
  {image ? (
    <Image source={{ uri: image }} style={styles.preview} />
  ) : (
    <Text style={{ color: "#aaa" }}>Tap to upload screenshot</Text>
  )}
</TouchableOpacity>
