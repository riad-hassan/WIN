...
// takeoff progress
let takeoffProgress = phase === "takeoff"
  ? planeX / (app.screen.width * 0.75)
  : 1

// base zoom
let zoomBase = 1 + takeoffProgress * 0.25

// multiplier zoom
let zoomBoost = phase === "cruise"
  ? Math.pow(multiplier, 1.2) * 0.01
  : 0

// final zoom
let targetZoom = zoomBase + zoomBoost

// breathing effect
targetZoom += Math.sin(Date.now() * 0.003) * 0.01

// clamp
targetZoom = Math.min(targetZoom, 1.2)

// smooth zoom
camera.scale.x += (targetZoom - camera.scale.x) * 0.08
camera.scale.y += (targetZoom - camera.scale.y) * 0.08



// ---------- SHAKE (SEPARATE OFFSET) ----------

// intensity
let intensity = Math.min(8, multiplier * 0.5)

// decay
let decay = Math.max(0, 1 - multiplier * 0.05)

// directional (top-left feel)
let shakeX = (-intensity * decay * 0.7) + (Math.random() - 0.5) * 0.5
let shakeY = (-intensity * decay * 0.5) + (Math.random() - 0.5) * 0.5



// ---------- APPLY CAMERA ----------

// IMPORTANT: pivot fix (set once ideally, but safe here)
camera.pivot.set(app.screen.width / 2, app.screen.height / 2)

// smooth position instead of direct jump
camera.position.x += ((app.screen.width / 2 + shakeX) - camera.position.x) * 0.1
camera.position.y += ((app.screen.height / 2 + shakeY) - camera.position.y) * 0.1
...
// ---------- CRUISE ----------
 ...
 if(points.length === 1){
  points[0] = { x: planeXFixed, y: startY }
}
  ...
  // smooth rotation
 let targetRotation = -0.2 + Math.min(0.35, Math.pow(t, 0.8) * 0.2)

// smooth easing
plane.rotation += (targetRotation - plane.rotation) * 0.08
}
