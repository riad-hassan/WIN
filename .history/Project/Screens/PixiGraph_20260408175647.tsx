...
// ---------- PLANE ----------
const planeTexture = PIXI.Texture.from("https://i.ibb.co/rRnPGGCq/plane.png")
...
planeTexture.baseTexture.on("loaded", () => {
  const scale = targetWidth / plane.texture.width
  plane.scale.set(scale)
})

// 🔥 better position
plane.x = app.screen.width * 0.6
plane.y = app.screen.height * 0.45
...
// ---------- CAMERA ZOOM ----------
let zoom = 1 + Math.log(multiplier) * 0.008
zoom = Math.min(zoom, 1.3)   // 🔥 clamp hard

camera.scale.set(zoom)

// 🔥 follow plane
camera.pivot.set(plane.x, plane.y)
