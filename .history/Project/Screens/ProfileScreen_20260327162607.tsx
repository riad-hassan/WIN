...
useFocusEffect(
  useCallback(() => {
    const loadKycStatus = async () => {
      ...
      // 🔥 Balance progress
       // 🔹 Funds Step (balance অনুযায়ী)
      const perc = Math.min(balance / maxBalance, 1); // 0–1
      setFundsProgress(perc);
    };
...
<View style={styles.progressBg}>
  ...
  {kycProgress === 1 && (
    <Animated.View
      style={[
        styles.progressFill,
        {
          backgroundColor: "#22c55e", // green for funds
          width: `${fundsProgress * 100}%`,
          position: "absolute",
          top: 0,
        },
      ]}
  ...
  <View style={styles.stepsRow}>
    {/* Basic Step */}
    <View style={progress >= 1 ? styles.step : styles.activeStep}>
      <MaterialIcons name="person" size={20} color="#facc15" />
      <Text style={progress >= 1 ? styles.stepText : styles.activeText}>
        Basic
      </Text>
    </View>

    {/* Funds Step */}
    <View style={progress === 1 ? styles.activeStep : styles.step}>
      <MaterialIcons name="account-balance-wallet" size={20} color="#facc15" />
      <Text style={progress === 1 ? styles.activeText : styles.stepText}>
        Funds
      </Text>
    </View>
  </View>
