...
const blastImg = require("./assets/blast.png");
...
const [line,setLine] = useState("M0 240")
const [area,setArea] = useState("M0 240")
const [plane,setPlane] = useState({x:0,y:240})
...
const [crashValue,setCrashValue] = useState(0) // betting point crash hole dykabe
...
const PLANE_OFFSET_Y = 90;   // plane up
const LINE_OFFSET_Y = 25;    // line একটু নিচে থাকবে
const PLANE_LEAD_X = -17;     // plane সামনে থাকবে
...
const generateRandomHistory = () => {
  const arr = []
  for (let i = 0; i < 15; i++) {
    const num = (Math.random() * 5 + 1).toFixed(2)
    arr.push(Number(num))
  }
  return arr
}
const [history,setHistory] = useState(generateRandomHistory())  
...
// 🔥 queued bet auto place
if(queuedBet1){
  const amount = parseInt(bet1)
  if(amount <= balance){
    setBalance(prev => prev - amount) // ✅ MUST
    setBetPlaced1(true)
...
crashPoint.current = 1.6 + Math.random()*8
 setMultiplier(1)
 setCrashed(false)
...
let points = [`M${x} ${y - LINE_OFFSET_Y}`]
 const animate = () => {

x += 14

const curveGrowth = 0.8 + multiplier * 0.35
y -= curveGrowth

if (y < 130) y = 130

points.push(`L${x} ${y - LINE_OFFSET_Y}`)

const path = points.join(" ")

setLine(path)
setArea(path + ` L${x} ${240 - LINE_OFFSET_Y} L0 ${240 - LINE_OFFSET_Y}`)
setPlane({x: x + PLANE_LEAD_X, y})
// plane shake
if(x > width*0.45){
y += Math.sin(Date.now()/120)*1.5
}
setTrail(prev=>{
const newTrail = [...prev,{x,y:y-45}] //plane upore nice kora
return newTrail.slice(-20)
})

setMultiplier(prev => {

const next = prev + (0.03 + prev * 0.015)  // point baraty
...
const crash=()=>{
 ...
 setCrashValue(crashPoint.current.toFixed(2))
 setCrashed(true)
setWaiting(true)   // 🔥 waiting start
...
setMyBets(prev =>
  prev.map(item =>
    item.win === "0"
      ? {
          ...item,
          multi: `${crashPoint.current.toFixed(2)}x`,
          win: "0"
        }
      : item
  )
...
// history er jonno 
 ...
 const newItem = parseFloat(crashPoint.current.toFixed(2))
  // যদি আগেরটার সাথে একই হয় তাহলে add করবে না
 if(prev[0] === newItem){
  return prev
 }
...
setResult(`You Lost ${betAmount1} BDT`)
}

if(betPlaced2 && !cashedOut2){
setResult(`You Lost ${betAmount2} BDT`)
}
...
const fakePlayers=()=>{
 ...
 playerTimer.current = setInterval(()=>{
  ...
  const betAmount=[1000,2500,500,6700,5600,7645,20000,12500][Math.floor(Math.random()*8)]

  const multi=(1+Math.random()*3).toFixed(2)

  const win=(betAmount*multi).toFixed(2)

  setPlayers(p=>{
   const newList=[{id:String(id),bet:betAmount,multi:`${multi}x`,win}]
   const combined=[...newList,...p].slice(0,15)
   return combined
  })
