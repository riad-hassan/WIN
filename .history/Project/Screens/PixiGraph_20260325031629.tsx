...
plane.rotation += (targetRotation - plane.rotation) * 0.1
  ...
  if(planeX >= targetX){
    planeX = targetX
    y = targetY
plane.x = targetX   // 🔥 freeze position
...
points.forEach(p => {
  ...
  const baseY = app.screen.height - 40   // ground
const maxHeight = 120                  // কতটুকু উঠতে পারবে

let normalized = 1 - (1 / (1 + Math.pow(multiplier, 1.5)))  

y = baseY - (normalized * maxHeight)
y = Math.max(app.screen.height - 140, y)
  
// 🔥 plane fixed (illusion)
  const planeXFixed = app.screen.width * 0.75
plane.x = planeXFixed
  plane.y = targetY
