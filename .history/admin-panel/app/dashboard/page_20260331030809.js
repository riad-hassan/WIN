...
// ================= RefferSetting =================
const saveRefferSettings = async () => {
  setRefferLoading(true);

  const updates = [
    { key: "referral_link", value: refferData.referral_link },
    { key: "referral_bonus", value: refferData.referral_bonus },
    { key: "referral_commission", value: refferData.referral_commission },
    { key: "referral_status", value: refferData.referral_status },
  ];

  for (let item of updates) {
    await supabase.from("Reffer").upsert(item, { onConflict: "key" });
  }

  alert("Referral Settings Updated Successfully!");
  setRefferLoading(false);
  load();
};
