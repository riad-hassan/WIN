...
const getTabs = () => {
  ...
  if (currentUser.role === "admin") {
    return [...baseTabs, "payments", "notify"];
  }
...
{/* 2️⃣ Tab Content */}
 <div className="tab-content">
  ...
  // ✅ Only add balance if Approved
  if (newStatus === "Approved") {
    const { data: userData, error: userError } = await supabase
      .from("users")
      .update({
        balance: supabase.raw("balance + ?", [d.amount])
      })
      .eq("id", d.user_id)
      .select();

    if (userError) {
      console.error("Balance update error:", userError.message);
      alert("Balance update failed!");
      return;
    }

    console.log("Balance updated:", userData);
  }
