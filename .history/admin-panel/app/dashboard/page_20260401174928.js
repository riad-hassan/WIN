...
const filterByAgent = (me, users, data, type) => {
 ...
 if (type === "withdraws") {
  return data.filter((w) =>
    users.some((u) => {
      // Case 1: user_id match (Best)
      if (w.user_id) {
        return u.id === w.user_id && u.referral === me.uid;
      }

      // Case 2: fallback uid match (Old Data Support)
      return u.uid === w.uid && u.referral === me.uid;
    })
  );
}
...
const fetchAppPasswords = async () => {
  ...
  if (error) {
    console.log("Fetch app_passwords error:", error.message);
    return;
  }
...
<div className="mt-4">
  <button
    ...
    className="w-full bg-red-600 py-2 rounded-lg hover:bg-red-700 transition text-white font-semibold"
  >
    Delete User
  </button>
</div>
      ...
      <div className="space-y-3">
        ...
        {appPasswords.map((p) => (
          ...
          >
            ...
            {/* 🗑️ Delete Button */}
  <button
    onClick={() => deleteAppPassword(p.id)}
