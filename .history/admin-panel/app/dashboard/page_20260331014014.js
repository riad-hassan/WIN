...
{tab === "withdraw" && (
  <div className="space-y-4">
    ...
    {withdraws.map((w) => {
      ...
      const [status, setStatus] = useState(w.status || "Pending");
      ...
      const handleAction = async (newStatus) => {
        ...
        await supabase.from("notifications").insert({
          ...
          message: `Your withdraw request of ${w.amount} ৳ has been ${newStatus}. Note: ${adminNote}`,
          user_id: w.uid,
          type: "User"
        });

        setStatus(newStatus); // Live update status badge
        setModalOpen(false);
        alert(`Withdraw ${newStatus} & notification sent!`);
      };
