...
const [systemStatus, setSystemStatus] = useState({
  referral_status: "Loading...",
  telegram_link: "Loading...",
  whatsapp_link: "Loading...",
});
...
const loadSystemStatus = async () => {
  ...
  setSystemStatus({
    referral_status: refData?.value || "off",
    telegram_link: telegramData?.value || "Not Set",
    whatsapp_link: whatsappData?.value || "Not Set",
  });
};
...
const todayWithdrawsTotal = withdraws
  .filter(w => new Date(w.created_at).toDateString() === today)
...
const latestUsers = users
  ...
  <div className="space-y-6">
    ...
    {/* TODAY REPORT + SYSTEM STATUS */}
<div className="grid grid-cols-1 lg:grid-cols-3 gap-5">

  {/* Today Report */}
  <div className="bg-gray-950 p-6 rounded-2xl border border-gray-800 shadow-lg">
    <h3 className="text-xl font-bold text-white mb-4">📅 Today Report</h3>

    <div className="space-y-3">
      <div className="flex justify-between bg-gray-900 p-3 rounded-xl border border-gray-800">
        <p className="text-gray-300 text-sm">Today Deposits</p>
        <p className="text-green-400 font-bold">
          ৳ {todayDepositsTotal.toLocaleString()}
        </p>
      </div>

      <div className="flex justify-between bg-gray-900 p-3 rounded-xl border border-gray-800">
        <p className="text-gray-300 text-sm">Today Withdraws</p>
        <p className="text-red-400 font-bold">
          ৳ {todayWithdrawsTotal.toLocaleString()}
        </p>
      </div>
      ...
      <div className="flex justify-between bg-gray-900 p-3 rounded-xl border border-gray-800">
        <p className="text-gray-300 text-sm">Pending Withdraws</p>
        <p className="text-yellow-400 font-bold">
          {pendingWithdraws}
        </p>
      </div>
  ...
  <div className="bg-gray-950 p-6 rounded-2xl border border-gray-800 shadow-lg">
    ...
    <div className="space-y-3">
      ...
      <div className="flex justify-between bg-gray-900 p-3 rounded-xl border border-gray-800">
        <p className="text-gray-300 text-sm">Referral System</p>

        <span className={`px-3 py-1 rounded-full text-xs font-bold ${
          systemStatus.referral_status === "on"
            ? "bg-green-600 text-white"
            : "bg-red-600 text-white"
        }`}>
          {systemStatus.referral_status?.toUpperCase()}
        </span>
      </div>

      <div className="bg-gray-900 p-3 rounded-xl border border-gray-800">
        <p className="text-gray-300 text-sm mb-1">Telegram Link</p>
        <p className="text-white text-xs break-all">
          {systemStatus.telegram_link}
        </p>
      </div>

      <div className="bg-gray-900 p-3 rounded-xl border border-gray-800">
        <p className="text-gray-300 text-sm mb-1">WhatsApp Link</p>
        <p className="text-white text-xs break-all">
          {systemStatus.whatsapp_link}
        </p>
      </div>
  ...
  {/* Latest Users */}
  <div className="bg-gray-950 p-6 rounded-2xl border border-gray-800 shadow-lg">
    <h3 className="text-xl font-bold text-white mb-4">👤 Latest Users</h3>
    ...
    {latestUsers.length === 0 ? (
      ...
      <div className="space-y-3">
        {latestUsers.map((u) => (
          <div
            key={u.id}
            className="bg-gray-900 p-3 rounded-xl border border-gray-800 hover:bg-gray-800 transition"
          >
