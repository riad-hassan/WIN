...
useEffect(() => {
  ...
  // Agent হলে শুধু তার referral user দেখাবে
  if (me.role === "agent") {
    filteredUsers = filteredUsers.filter((u) => u.referral === me.uid);
  }
  ...
  if (me.role === "agent") {
    filteredDeposits = filteredDeposits.filter((d) =>
      filteredUsers.some((u) => u.id === d.user_id)
    );
  }
  ...
  if (me.role === "agent") {
    filteredWithdraws = filteredWithdraws.filter((w) =>
      filteredUsers.some((u) => u.uid === w.uid)
    );
  }
  ...
  // balances
  const balances = {};
  filteredUsers.forEach((u) => (balances[u.id] = u.balance || 0));
...
const rejectedWithdraws = withdraws.filter(w => w.status === "Rejected").length;
  ...
  {[
    ...
    "deposits",
    "withdraw",
    ...(currentUser?.role === "admin" ? ["payments", "notify"] : []),
  ].map((t) => (
    <button
      key={t}
      onClick={() => setTab(t)}
...
{/* 2️⃣ Tab Content */}
  <div className="tab-content">
    ...
    {tab === "deposits" && <DepositsComponent />}
    {tab === "withdraw" && <WithdrawComponent />}
    
    {/* 3️⃣ Admin Only Tabs */}
    {tab === "payments" && currentUser?.role === "admin" && <PaymentsComponent />}
    {tab === "notify" && currentUser?.role === "admin" && <NotifyComponent />}
  </div>
