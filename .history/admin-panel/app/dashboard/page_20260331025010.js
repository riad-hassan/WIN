...
// Load payments from Supabase
  ...
  <div className="space-y-4">
    ...
    {deposits.map((d) => {
      ...
      const [status, setStatus] = useState(d.status || "Pending");

      const handleAction = async (newStatus) => {
        // Update deposit status and admin note
        ...
        await supabase.from("notifications").insert({
          ...
          message: `Your deposit of ${d.amount} ৳ has been ${newStatus}. Note: ${adminNote}`,
          user_id: d.user_id,
          type: "User"
        });
      ...
      return (
        <div key={d.id} className="bg-gray-800 p-5 rounded-xl shadow-md border border-gray-700 hover:shadow-lg transition relative">
